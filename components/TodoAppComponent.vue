...
<template>
    <div class="grid-layout space-y-6">
        ...
        <div class="max-h-64 overflow-y-auto">
            <div v-for="task in todos" :key="task.uid"
                class="px-2 flex-between w-64 h-12 mb-2 bg-white rounded-md flex-center md:h-14 md:w-96">
                <div class="flex-center space-x-2">
                    <input type="checkbox" :checked="task.completed" class="accent-green-600" @change="handleCompleted(task)" />
                    <p :class="[task.completed ? 'line-through text-black' : 'text-black font-bold']">{{ task.todo }}</p>
                </div>
